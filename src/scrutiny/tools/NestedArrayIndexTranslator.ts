 ...
 */
export default class NestedArrayIndexTranslator {
    ...
    translate(path: string[], index: number): number {
        let value = index;
        for (const alteration of this.alterations) {
            if (pathMatches(alteration.path, path) && offsetApplies(index, alteration)) {
...
function offsetApplies(index: number, alteration: Alteration) {
    if (alteration.offset > 0) {
        ...
        if (index > alteration.index) {
            ...
            console.warn('offsetApplies is requesting the index that was removed', alteration);
        }
    }
    return false;
}
