...
export function walkObject<T>(obj: any, keys: string[], onEach: WalkObjectOnEachHandle<T>): T {
    let level = obj;
    const path: string[] = [];
    for (let i = 0; i < keys.length; i++) {
        ...
        if (key in level) {
            ...
            throw new Error(`Failed to walk object, key ${key} is missing from value at ${path.join('/')}`);
        }
    }
    return undefined as T;
}

export function walkObjectGetValue<T>(obj: any, keys: string[], defaultValue: T): T {
