import { useCallback, useContext, useMemo, createContext, PropsWithChildren } from 'react';
import NestedArrayIndexTranslator from './NestedArrayIndexTranslator';
import { walkObject, walkObjectDeleteValue, walkObjectGetValue, walkObjectSetValue } from './walkObject';
 ...
 * @returns
 */
export function NestedStateFolder({ children, name }: { name: string | string[] } & PropsWithChildren) {
    const context = useContext(NestedStateContext);
    return (
        <NestedStateContext.Provider
            value={{
...
export function useNestedStatePath(key?: string | string[], pathSeparator?: string) {
...
export interface UseNestedStateOptions {
 ...
 * @returns
 */
function nestedStateDispatchHandle(state: any, ...actions: NestedStateDispatchAction[]) {
            ...
            case 'move':
                {
                    ...
                    const value = walkObject(workState, action.source, ({ isLast, path, parent, key, $return, $setKey }) => {
                        const thisIsAnArray = isArray(parent);
                        const myKey = thisIsAnArray ? idxTranslator.translate(path, parseInt(key, 10)) : key;
                        if (isLast) {
                            if (thisIsAnArray) {
                    ...
                    if (typeof value === 'undefined') {
                        // do we need to abort?
                        console.warn('nestedStateDispatch value moved is undefined', action, state);
                    }

                    walkObject(
                        state,
                        isArray(action.destination) ? action.destination : action.destination.path,
                        ({ parent, path, $setKey, key, isLast, isMissing }) => {
                            ...
                            if (isLast) {
                                if (isArray(action.destination) || action.destination.location === 'replace') parent[myKey] = value;
                                else {
                                    switch (action.destination.location) {
                                        ...
                                        case 'before': {
                                            ...
                                            const destIndex =
                                                idxTranslator.translate(path, parseInt(key, 10)) +
                                        ...
                                        default: {
                                            throw new Error(
                                                `nestedStateDispatch invalid destination location ${action.destination.location}`,
                                            );
                                        }
                                    }
                                }
...
export function useNestedState<T>(
    ...
    key: string | string[],
    defaultValue: T,
    opts?: UseNestedStateOptions,
): [T, { (value: T): void }, NestedStateDispatch, NestedStatePath] {
    const context = useContext(NestedStateContext);
    if (!(store in context.stores)) {
        throw new Error(`Unknown store ${store} in NestedStateContext. Please specify it with a NestedStateStore component`);
    }
    ...
    const absolutePath = opts?.absolutePath ?? (typeof key === 'string' && key.substring(0, 1) === pathSeparator);
    const splitKey = useCallback(
        (inkey: string | string[]) => {
            if (typeof inkey === 'string') {
                return pathSeparator ? inkey.split(pathSeparator) : [inkey];
            }
            return inkey;
        },
        [pathSeparator],
    ...
    const doClear = useCallback(
        (newValue: any) => {
            return clearOnDefault && JSON.stringify(newValue) === serializedDefaultValue;
        },
        [clearOnDefault, serializedDefaultValue],
    );
    const nestedStatePath: NestedStatePath = useMemo(
        () => [...(absolutePath ? [] : context.prefixKeys.map(splitKey)), ...splitKey(key)].flat(),
